...
from models import ReleaseFocus, Project, UserProject
...
class kolektiMixin(LoginRequiredMixin, TemplateResponseMixin, kolektiBase):
    ...
    def projects(self):
        ...
        for up in UserProject.objects.filter(user = self.request.user):
            ...
            try:
                ...
                synchro = SynchroManager(self.kolekti_projectpath)
    ...
    def get_toc_edit(self, path):
        ...
        xsl = self.get_xsl('django_toc_edit', extclass = PublisherExtensions, lang = self.kolekti_userproject.srclang)
        try:
            etoc = xsl(xtoc)
        except:
            import traceback
    ...
    def get_topic_edit(self, path):
        return self.read(path.replace('{LANG}',self.kolekti_userproject.srclang))
    ...
    def get_job_edit(self, path):
        ...
        xsl = self.get_xsl('django_job_edit', extclass=PublisherExtensions, lang=self.kolekti_userproject.srclang)
        try:
    ...
    def language_activate(self, language):
        # get userdir
        self.kolekti_userproject.srclang = language
        self.kolekti_userproject.save()
...
class HomeView(kolektiMixin, View):
    ...
    def get(self, request):
        context = self.get_context_data()
        if not self.kolekti_userproject:
...
class ProjectsView(kolektiMixin, View):
    ...
    def checkout_project(self, request):
        ...
        sync = SVNProjectManager(os.path.join(settings.KOLEKTI_BASE,self.user.username),username,password)
        if project_url=="":
...
class ReleaseStateView(kolektiMixin, TemplateView):
    def get(self, request):
        ...
        lang = request.GET.get('lang', self.kolekti_userproject.srclang)
...
#        xjob = self.parse(jobpath)
        try:
            p = publish.ReleasePublisher(release_path, self.kolekti_projectpath, langs=langs)
...
class VariablesUploadView(kolektiMixin, TemplateView):
    def post(self, request):
        ...
        if form.is_valid():
            ...
            uploaded_file = request.FILES[u'upload_file']
            path = request.POST['path']
            converter = OdsToXML(self.kolekti_projectpath)
...
class VariablesODSView(kolektiMixin, View):
    def get(self, request):
        ...
        odsfile = StringIO()
        converter = XMLToOds(self.kolekti_projectpath)
...
class ImportView(kolektiMixin, TemplateView):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            importer = Importer(self.kolekti_projectpath, lang=self.kolekti_userproject.srclang)
...
class ImportTemplateView(kolektiMixin, TemplateView):
    def get(self, request):
        ...
        odsfile = StringIO()
        tplter = Templater(self.kolekti_projectpath)
        tplter.generate("/sources/"+self.kolekti_userproject.srclang+"/templates/"+template, odsfile)
        response = HttpResponse(odsfile.getvalue(),                            
                                content_type="application/vnd.oasis.opendocument.spreadsheet")
        response['Content-Disposition']='attachement; filename="%s"'%filename
        odsfile.close()
...
class SettingsJsView(kolektiMixin, TemplateView):
    def get(self, request):
        ...
        """%(self.kolekti_userproject.srclang, self.kolekti_userproject.project.name)
...
class DraftView(PublicationView):
    def post(self,request):
        ...
        try:
            ...
            p = publish.DraftPublisher(self.kolekti_projectpath, lang=self.kolekti_userproject.srclang)
...
#        pubtitle= request.POST.get('pubtitle')
        ...
        try:
            ...
            xjob.getroot().set('pubdir',pubdir)
            return StreamingHttpResponse(self.format_iterator(self.release_iter(self.kolekti_projectpath, tocpath, xjob)))
    ...
    def release_iter(self, projectpath, tocpath, xjob):
        ...
        p = publish.ReleasePublisher(release_dir, projectpath, langs=[self.kolekti_userproject.srclang])
...
#                                         "meta":topicmeta
    ...
    def post(self,request):
        try:
            ...
            # xtopic = self.parse(path.replace('{LANG}',self.kolekti_userproject.srclang))
...
class TopicCreateView(kolektiMixin, View):
    ...
    def post(self, request):
        try:
            modelpath = '/sources/'+ self.kolekti_userproject.srclang + "/templates/" + request.POST.get('model')
...
class SearchView(kolektiMixin, View):
    ...
    def get(self, request):
        ...
        s = searcher(self.kolekti_projectpath)
        results = s.search(q)
...
class SyncView(kolektiMixin, View):
    ...
    def get(self, request):
        ...
        try:
            from kolekti.synchro import SynchroManager
            sync = SynchroManager(self.kolekti_projectpath)
            context.update({
...
class projectStaticView(kolektiMixin, View):
    def get(self, request, path):
        return serve(request, path, self.kolekti_projectpath)
