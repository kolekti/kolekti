...
<xsl:stylesheet version="1.0"
        ...
        exclude-result-prefixes="html k">
  ...
  <xsl:variable name="encoding">
    <xsl:choose>
      <xsl:when test="$publang=document('../encodings.xml')//k:bindings/k:bind/k:lang/@code">
        <xsl:value-of select="document('../encodings.xml')//k:bindings/k:bind[k:lang/@code=$publang]/@encoding"/>
      </xsl:when>
  ...
  <xsl:template match="html:div[@class='module'][html:div[@class='moduleinfo']/following-sibling::html:*]">
    ...
    <exsl:document href="file://{$upubdir}/{$label}/{$filename}" method='html' indent="yes" doctype-public="-//W3C//DTD HTML 4.01 Transitional//EN" encoding="{$encoding}">
      <html>
	<head>
