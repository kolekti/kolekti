...
class plugin(pluginBase.plugin):
    ...
    def postpub(self):
        ...
        """
        res = []
        logging.debug( "html out  : %s %s"%(self.assembly_dir,self.publication_dir))
        ...
        self.xwrite(doc, "%s/%s.html"%(self.publication_plugin_dir,self.publication_file))
        res.append({'type':"pivot",
                    "label":"%s"%self.publication_file,
                    "url": "%s/%s.html"%(self.publication_plugin_dir,self.publication_file),
                    "ET": doc})
        return res
