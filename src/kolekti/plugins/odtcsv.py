...
from kolektiserver.publication.plugins._odtcsv import odtpdf
...
class plugin(pluginBase.plugin):
    ...
    def postpub(self):
        ...
        try:
            ...
            doc=xslt(tmeta,
                     pivot="'%s_tmp'"%self.publisher.model.abstractIO.local2unicode(pivfile))
        except:
            dbgexc()
        ...
        try:
            doc=xslt(template,
                     ...
                     styles="'%s_tmpstyles'"%urllib.quote(os.path.join(pubpath,pivfile)),
                     mapping="'%s'"%mapfile)
        except:
            dbgexc()
    ...
    def generate_csv(self, pdf):
        ...
        try:
            csv = "%s/%s.csv" %(self.publisher.model.pubpath.rsplit(os.sep, 3)[0], self.publisher.model.pivname)
