...
#        logger.debug("index %s %s"%(restype, path))
    ...
    def delete_resource(self, path):
        with self.ix.writer() as writer:
            writer.delete_by_term('path',path.decode('utf-8'))


class Searcher(SearchUtils):
    def __init__(self, projectspath, projectdir):
        super(Searcher, self).__init__(projectspath, projectdir)
        indexpath = os.path.join(projectspath, '.index', projectdir)
        self.ix = index.open_dir(indexpath)
    ...
    def search(self, query, page=1):
        ...
        with self.ix.searcher() as searcher:
            ...
            for r in results:
                res= dict(r)
                if res['type'] == 'topic':
                    contents = self.xhtml_extract(res['path'])[1]
                    res.update({'hl':r.highlights("content", text = contents)})
                yield res
