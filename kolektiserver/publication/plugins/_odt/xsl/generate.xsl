...
<xsl:output indent="yes"/>
  ...
  <xsl:template match="html:*" mode="revmarks">
   <xsl:variable name="linkid" select="//html:a[(@class='voirpage' or @class='voirtexte')and @href=concat('#', ../@id)]" />
  ...
  <xsl:template match="html:a[@class='voirtexte']">
   <xsl:variable name="href" select="substring(@href, 2)" />
   <text:reference-ref text:reference-format="text" text:ref-name="{$href}">0</text:reference-ref>
  </xsl:template>
...
-->
  ...
  <xsl:template match="html:img[@class]">
    ...
    <xsl:variable name="anchor">
      <xsl:call-template name="img-anchor"/>
    </xsl:variable>

    <draw:frame draw:style-name="fr{1+count(preceding::html:img)}" draw:name="images{1+count(preceding::html:img)}"  draw:z-index="0" text:anchor-type="{$anchor}">
      <xsl:if test="$anchor = 'page'">
	<xsl:variable name="class" select="@class"/>
	<xsl:if test="$mappings/map[@element='img'][(@class and &inclass;)]/frame/@anchor-page-number">
	  <xsl:attribute name="text:anchor-page-number">
	    <xsl:value-of select="$mappings/map[@element='img'][(@class and &inclass;)]/frame/@anchor-page-number"/>
	  </xsl:attribute>
	</xsl:if>
      </xsl:if>
  ...
  <xsl:template name="img-anchor">
    <xsl:variable name="class" select="@class"/>
    <xsl:choose>
      <xsl:when test="$mappings/map[@element='img'][(@class and &inclass;)]/frame/@anchor">
	<xsl:value-of select="$mappings/map[@element='img'][(@class and &inclass;)]/frame/@anchor"/>
      </xsl:when>
  ...
  <xsl:template match="html:div[@class]">
	...
	<xsl:variable name="frameanchor">
	  ...
	  <xsl:when test="$frameanchor = 'page'">
	    <!--get pagenum for anchor -->
	    
	    <xsl:variable name="frameanchorpg">
	      <xsl:choose>
		<xsl:when test="$mappings/map[@element='div'][(@class and &inclass;)]">
		  <xsl:choose>
		    <xsl:when test="$mappings/map[@element='div'][(@class and &inclass;)]/frame/@anchor-page-num">
		      <xsl:value-of select="$mappings/map[@element='div'][(@class and &inclass;)]/frame/@anchor-page-num"/>
		    </xsl:when>
		    <xsl:otherwise>1</xsl:otherwise>
		  </xsl:choose>
		</xsl:when>
		<xsl:when test="$mappings/map[@element='div'][not(@class)]/frame/@anchor-page-num">
		  <xsl:value-of select="$mappings/map[@element='div']/frame/@anchor-page-num"/>
		</xsl:when>
		<xsl:otherwise>1</xsl:otherwise>
	      </xsl:choose>
	    </xsl:variable>
	      ...
	      <xsl:choose>
		<xsl:when test="$mappings/map[@element='div'][(@class and &inclass;)]/frame/@anchor-page-number">
		  <xsl:attribute name="text:anchor-page-number">
		    <xsl:value-of select="$mappings/map[@element='div'][(@class and &inclass;)]/frame/@anchor-page-number"/>
		  </xsl:attribute>
		</xsl:when>
		<xsl:when test="$mappings/map[@element='div'][not(@class)]/frame/@anchor-page-number">
		  <xsl:attribute name="text:anchor-page-number">
		    <xsl:value-of select="$mappings/map[@element='div'][not(@class)]/frame/@anchor-page-number"/>
		  </xsl:attribute>
		</xsl:when>
	      </xsl:choose>
