...
function a_search(inputid) {
    var words = $('#'+inputid)[0].value;
    var restot={};
    var rescount={};
    var restab={};
    ...
    var count = 1;
    var htmlbuf = '';
    var searchl = $("#search-results-content");
    searchl.html("");
    for (r in ressort) {
        res=ressort[r];
        if (res) {
            htmlbuf += '<li><p><a href="'+res+'.html"><strong>'+modcodes[res+".html"]+'</strong><br /><em>'+show_search_words(res,false)+'</em></a></p></li>';
            if(count > maxres)
                break;
            count++;
        }
    ...
    if(htmlbuf != '') {
        searchl.html('<ul>'+htmlbuf+'<ul>');
    } else {
	searchl.html("<ul><li><p>Aucun résultat pour <em>"+words+"</em></p></li><ul>");
    }
    return
    if(htmlbuf != '') {
